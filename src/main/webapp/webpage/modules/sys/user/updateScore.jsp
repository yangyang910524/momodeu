...
<head>
	...
	function save() {
		...
		if(!isValidate){
			...
			jp.loading();
			jp.post("${ctx}/sys/user/updateScore",$('#inputForm').serialize(),function(data){
				if(data.success){
	...
	<form:form id="inputForm" modelAttribute="user"  method="post" class="form-horizontal">
		...
		<table class="table table-bordered">
		   <tbody>
			  ...
			  <tr>
			  <tr>
				  <td class="active"><label class="pull-right">调整前积分:</label></td>
				  <td><input class="form-control" readonly="readonly" value="${user.score}"/></td>
				  <td class="active"><label class="pull-right"><font color="red">*</font>调整后积分:</label></td>
