...
$(document).ready(function() {
	$('#noticeTable').bootstrapTable({
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                       ...
                       view(row.id);
                        ...
                        jp.confirm('确认要删除该消息公告记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/notice/notice/delete?id="+row.id, function(data){
                   	  		if(data.success){
	  ...
	  $('#noticeTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
            ...
            $('#view,#edit').prop('disabled', $('#noticeTable').bootstrapTable('getSelections').length!=1);
        });
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					 jp.downloadFile('${ctx}/notice/notice/import/template');
				  },
			    btn2: function(index, layero){
				        var iframeWin = layero.find('iframe')[0]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
						iframeWin.contentWindow.importExcel('${ctx}/notice/notice/import', function (data) {
							if(data.success){
								jp.success(data.msg);
								refresh();
							}else{
	 ...
	 $("#export").click(function(){//导出Excel文件
            ...
            var sortName = $('#noticeTable').bootstrapTable("getOptions", "none").sortName;
            var sortOrder = $('#noticeTable').bootstrapTable("getOptions", "none").sortOrder;
            var values = "";
			...
			jp.downloadFile('${ctx}/notice/notice/export?'+values);
	  })
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该消息公告记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/notice/notice/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#noticeTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function add(){
		jp.go("${ctx}/notice/notice/form/add");
	}
  ...
  function edit(id){
	  ...
	  jp.go("${ctx}/notice/notice/form/edit?id=" + id);
  }
