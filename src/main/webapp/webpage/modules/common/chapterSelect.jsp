			...
			//初始化表格
			  $('#table').bootstrapTable({
                  ...
                  contentType: "application/x-www-form-urlencoded",
	                ...
	                url: "${ctx}/course/courseInfo/chapterList?office.id=${officeid}",
				...
				<div class="accordion-inner">
					<form id="searchForm"  class="form form-horizontal well clearfix" >
						<div class="col-sm-2">
							...
							<input type="text" name="parent.name" maxlength="100"  class=" form-control" value="${parent.name}"/>
						</div>
						<div class="col-sm-2">
							<label class="label-item single-overflow pull-left" title="章节名称：">章节名称：</label>
							<input type="text" name="name" maxlength="100"  class=" form-control" value="${name}"/>
						</div>
						<div class="col-sm-2">
							...
							<select  class="form-control m-b" name="level">
								...
								<c:forEach items="${fns:getDictList('bae_course_level')}" var="l">
									<option value="${l.value}" <c:if test="${l.value eq level}">selected</c:if>>${l.label}</option>
								</c:forEach>
							</select>
						</div>
						<div class="col-sm-2">
